...
async function showAsyncConnection() {
  ...
  try {
    ...
    for(let i = 0; i < devList.length; i++) {
      if(devList[i].getAutoreconnect()) {
        connInfos.push(devList[i].toObject());
      }
    }
...
async function addAsyncConnection() {
  ...
  try {
    await connectMaster.addAsyncConnection(mainMenu.getGatewayID(), connInfos);
...
async function deleteAsyncConnection() {
  ...
  try {
    await connectMaster.deleteAsyncConnection(mainMenu.getGatewayID(), deviceIDs);
    await showAsyncConnection();
  } catch (err) {
